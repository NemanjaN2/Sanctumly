...
export default function ChatPage({ user, onLogout }) {
  ...
  const [sessionId] = useState(() => {
    ...
    // Fallback for legacy sessions (shouldn't happen after login update)
  ...
  const [ttsLanguage, setTtsLanguage] = useState('en-US')  // Default to English, toggle for Serbian
  const [showInstallPrompt, setShowInstallPrompt] = useState(false)
  const [deferredPrompt, setDeferredPrompt] = useState(null)
  const [isRecording, setIsRecording] = useState(false)
  const [mediaRecorder, setMediaRecorder] = useState(null)
  const [voiceMode, setVoiceMode] = useState(false)  // Voice conversation mode
  ...
  const personalities = [
    ...
    { value: 'therapist', label: 'Therapist', description: 'Mental health support' },
  ]
  ...
  const handleReadAloud = async (messageIndex, content) => {
    ...
    try {
      // Use manually selected language (default English, toggle for Serbian)
  ...
  const startVoiceRecording = async () => {
    ...
    try {
      ...
      // Start recording with timeslice to collect chunks every 250ms
      recorder.start(250)
      setMediaRecorder(recorder)
    } catch (err) {
      console.error('Microphone error:', err)
      ...
      <div className="flex-1 flex flex-col" style={{ maxWidth: '100%', margin: '0 auto' }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <button
                ...
                title={ttsLanguage === 'sr-RS' ? 'TTS: Serbian - Click for English' : 'TTS: English - Click for Serbian'}
              >
          ...
          >
            ...
            >
              <div style={{
                ...
                <h2 style={{ fontSize: '1.125rem', fontWeight: 600, color: 'var(--text-primary, #fff)' }}>
                  Download Apps
                </h2>
                <button 
                  onClick={() => setShowDownloadModal(false)}
              ...
              <div style={{ padding: '1.25rem' }}>
                <a
                  href="https://storage.googleapis.com/najdangpt-downloads/NajdanGPT.app.zip"
                  download
                  style={{
                    display: 'flex',
                    alignItems: 'center',
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 500, color: 'var(--text-primary, #fff)' }}>macOS App</div>
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary, #888)' }}>macOS 11+</div>
                  </div>
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--text-secondary, #888)" strokeWidth="2">
                    <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/>
                    <polyline points="7 10 12 15 17 10"/>
                    <line x1="12" y1="15" x2="12" y2="3"/>
                  </svg>
                ...
                <a
                  href="https://storage.googleapis.com/najdangpt-downloads/NajdanGPT.ipa"
                  download
                  style={{
                    display: 'flex',
                    alignItems: 'center',
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 500, color: 'var(--text-primary, #fff)' }}>iOS App (IPA)</div>
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary, #888)' }}>Requires sideloading</div>
                  </div>
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--text-secondary, #888)" strokeWidth="2">
                    <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/>
                    <polyline points="7 10 12 15 17 10"/>
                ...
                <a 
                  href="https://storage.googleapis.com/najdangpt-downloads/najdangpt.apk"
                  download
                  style={{
                    display: 'flex',
                    alignItems: 'center',
                ...
                >
                  <div style={{
                    ...
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="#d4af37">
                      <path d="M17.6 9.48l1.84-3.18c.16-.31.04-.69-.26-.85-.29-.15-.65-.06-.83.22l-1.88 3.24a11.43 11.43 0 0 0-8.94 0L5.65 5.67c-.19-.28-.54-.37-.83-.22-.3.16-.42.54-.26.85l1.84 3.18C4.8 11.16 3.5 13.84 3.5 16.5h17c0-2.66-1.3-5.34-2.9-7.02zM7 14.5c-.55 0-1-.45-1-1s.45-1 1-1 1 .45 1 1-.45 1-1 1zm10 0c-.55 0-1-.45-1-1s.45-1 1-1 1 .45 1 1-.45 1-1 1z"/>
                    </svg>
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 500, color: 'var(--text-primary, #fff)' }}>Android App (APK)</div>
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary, #888)' }}>Android 7.0+</div>
                  </div>
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--text-secondary, #888)" strokeWidth="2">
                    <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/>
                    <polyline points="7 10 12 15 17 10"/>
        ...
        {voiceMode && (
          ...
          >
            ...
            onClick={() => setTtsLanguage(prev => prev === 'en-US' ? 'sr-RS' : 'en-US')}
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={ttsLanguage === 'sr-RS' ? '#ffd700' : 'white'} strokeWidth="2">
                <polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/>
                <path d="M15.54 8.46a5 5 0 0 1 0 7.07"/>
              </svg>
              <span style={{ color: ttsLanguage === 'sr-RS' ? '#ffd700' : 'white', fontWeight: 600 }}>
                {ttsLanguage === 'sr-RS' ? 'Serbian' : 'English'}
              </span>
            </div>
            ...
            <div 
              style={{
                ...
                animation: voiceState === 'listening' ? 'pulse 1.5s ease-in-out infinite' : 
                          voiceState === 'thinking' ? 'pulse 0.8s ease-in-out infinite' :
                          voiceState === 'speaking' ? 'pulse 1s ease-in-out infinite' : 'none',
            ...
            >
              {voiceState === 'listening' ? (
                ...
                <svg width="80" height="80" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="1.5" style={{ animation: 'spin 2s linear infinite' }}>
