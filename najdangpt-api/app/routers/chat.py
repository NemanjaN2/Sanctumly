...
PREFERENCES: (how they like to be spoken to, what helps them — comma separated)"""
 ...
 logger.warning(f" Memory extraction returned empty summary for {username}")
 except Exception as e:
 logger.error(f" Memory save failed for {username}: {e}")
# ============================================================
# ROUTES
# ============================================================
...
async def chat_message(request: ChatRequest, db: Session = Depends(get_db)):
 ...
 image_b64 = clean_b64
 has_image = True
 logger.info(f" Image attached: {image_mime}, {len(clean_b64)} chars base64")
 except Exception:
 ...
 messages[0]["content"] = system_prompt
 logger.info(f" Search failed — injected anti-hallucination instruction")
...
async def clear_chat(session_id: str, db: Session = Depends(get_db)):
 ...
 db.commit()
 logger.info(f" Cleared {deleted} messages for session: {session_id[:30]}...")
